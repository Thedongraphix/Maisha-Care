...
This visual feedback should be in the form of a loading component that can render appearing and disappering text when for example an agentic function is running like when test recommendation is running it can render a brief loading icon followed by some text like, "agent creating test-suite" and other relevant texts depending on what is going on in the backend. this text should only appear when agentic functions are running, so when you get an SSE event of a workflow being triggered basically
